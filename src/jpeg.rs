// image_formats::jpeg
// fork of the code by Desmond Germans, 2019(https://github.com/des256/image_formats/)
...
const FOLDING: [u8; 64] = [
    56u8, 57, 8, 40, 9, 58, 59, 10, 41, 0, 48, 1, 42, 11, 60, 61, 12, 43, 2, 49, 16, 32, 17, 50, 3,
    44, 13, 62, 63, 14, 45, 4, 51, 18, 33, 24, 25, 34, 19, 52, 5, 46, 15, 47, 6, 53, 20, 35, 26,
    27, 36, 21, 54, 7, 55, 22, 37, 28, 29, 38, 23, 39, 30, 31,
];
const FC0: f32 = 1.0;
...
impl<'a> Reader<'a> {
    ...
    fn restock(&mut self) {
        while self.bit <= 24 {
            let b = if self.rp >= self.block.len() {
                0
            } else {
                jpeg_get8(self.block, &mut self.rp)
            }; // fill with 0 if at end of block
...
fn unpack_progressive_start_dc(
    reader: &mut Reader,
    coeffs: &mut [i32],
    dcht: &Table,
    dc: &mut i32,
...
fn unpack_progressive_start_ac(
    reader: &mut Reader,
    coeffs: &mut [i32],
    acht: &Table,
    start: u8,
...
fn unpack_progressive_refine_ac(
    reader: &mut Reader,
    coeffs: &mut [i32],
    acht: &Table,
    start: u8,
    ...
    if resint != 0 {
        *rescnt -= 1;
        if *rescnt == 0 {
            ...
            if (reader.block[tsp] == 0xFF)
                && ((reader.block[tsp + 1] >= 0xD0) && (reader.block[tsp + 1] < 0xD8))
            {
                tsp += 2;
                *rescnt = resint;
                dc[0] = 0;
...
fn convert_blocks(
    ...
    for i in 0..count {
        ...
        convert_block(
            &mut coeffs[i * 64..i * 64 + 64],
            &qtable[qt[(curp & 3) as usize]],
        );
        curp >>= 2;
    }
}
...
fn draw_rgb(image: &mut ImageBuffer, px: usize, py: usize, r: i32, g: i32, b: i32) {
    image.data[py * image.width * 4 + px * 4] = clamp(r as i32, 0, 255) as u8;
    image.data[py * image.width * 4 + px * 4 + 1] = clamp(g as i32, 0, 255) as u8;
    image.data[py * image.width * 4 + px * 4 + 2] = clamp(b as i32, 0, 255) as u8;
    image.data[py * image.width * 4 + px * 4 + 3] = 0xFF;
}
...
fn draw_macroblock_yuv422_normal(
    image: &mut ImageBuffer,
    x0: usize,
    y0: usize,
    width: usize,
...
#[cfg(feature = "nightly")]
fn draw_macroblock_yuv422_simd(
    image: &mut ImageBuffer,
    x0: usize,
    y0: usize,
    width: usize,
    ...
    fn draw_rgb_simd(image: &mut ImageBuffer, px: usize, py: usize, r: i32x4, g: i32x4, b: i32x4) {
        for i in 0..4 {
            image.data[py * image.width + px + i] =
                0xFF000000 | ((r[i] as u32) << 16) | ((g[i] as u32) << 8) | (b[i] as u32);
        }
    }

    fn i32x4v(a: i32, b: i32, c: i32, d: i32) -> i32x4 {
        i32x4::from_array([a, b, c, d])
    }
    fn i32x4s(a: i32) -> i32x4 {
        i32x4::from_array([a; 4])
    }
    ...
    fn draw_yuv_simd(image: &mut ImageBuffer, px: usize, py: usize, y: i32x4, u: i32x4, v: i32x4) {
        ...
        draw_rgb_simd(
            ...
            px,
            py,
            r.clamp(i32x4s(0), i32x4s(255)),
            g.clamp(i32x4s(0), i32x4s(255)),
            b.clamp(i32x4s(0), i32x4s(255)),
        );
    }
    ...
    for i in 0..height {
        for k in (0..width).step_by(4) {
            ...
            let sk3 = k3 & 7;
            let y = i32x4v(
                coeffs[by0 * 64 + i * 8 + sk0],
                coeffs[by1 * 64 + i * 8 + sk1],
                coeffs[by2 * 64 + i * 8 + sk2],
                coeffs[by3 * 64 + i * 8 + sk3],
            ) + i32x4s(128);
            let u = i32x4v(
                coeffs[128 + i * 8 + hk0],
                coeffs[128 + i * 8 + hk1],
        ...
        match marker {
            0xFFC0 | 0xFFC1 | 0xFFC2 => {
                ...
                if (components == 1) || (components == 3) {
                    // does not support RGBA or CMYK JPEGs
...
pub fn decode(src: &[u8]) -> Result<ImageBuffer, Error> {
    if from_be16(&src[0..2]) != 0xFFD8 {
        return Err(Error("Invalid JPEG 1".to_string()));
    }
    ...
    while sp < src.len() {
        let marker = from_be16(&src[sp..sp + 2]);
        let length = if marker != 0xFFD9 {
            from_be16(&src[sp + 2..sp + 4]) as usize
        } else {
            0
        };
        ...
        match marker {
            0xFFC0 | 0xFFC1 | 0xFFC2 => {
                // baseline sequential, extended sequential, progressive
                //println!("precision {}",src[sp + 4]);
                if src[sp + 4] != 8 {
                    return Err(Error("Invalid JPEG 2".to_string()));
                }
                ...
                if (components != 1) && (components != 3) {
                    return Err(Error("Invalid JPEG 3".to_string()));
                }
                let mut samp = [0u8; 3];
                let mut tsp = sp + 10;
                for i in 0..components {
                    if src[tsp] != i + 1 {
                        return Err(Error("Invalid JPEG 4".to_string()));
                    }
                ...
                if components == 3 {
                    if (samp[1] != 0x11) || (samp[2] != 0x11) {
                        return Err(Error("Invalid JPEG 5".to_string()));
                    }
                    ...
                    itype = match samp[0] {
                        ...
                        0x22 => TYPE_YUV420,
                        _ => {
                            return Err(Error("Invalid JPEG 6".to_string()));
                        }
                    };
                } else {
                    mbwidth = (width + 7) / 8;
            ...
            0xFFC4 => {
                ...
                while tsp < sp + length + 2 {
                    ...
                    if total >= 256 {
                        return Err(Error("Invalid JPEG 7".to_string()));
                    }
            ...
            0xFFDB => {
                ...
                while tsp < sp + length + 2 {
                    ...
                    if (d >> 4) != 0 {
                        for k in 0..64 {
                            qtable[n as usize][FOLDING[k as usize] as usize] =
                                from_be16(&src[tsp..tsp + 2]) as i32;
                            tsp += 2;
                        }
                    } else {
                        for k in 0..64 {
            ...
            0xFFE1 => {
                ...
                if header == 0x45786966 {
                    ...
                    tsp += 4; // skip 0x2A
                    tsp += (if le {
                        from_le32(&src[tsp..tsp + 4])
                    } else {
                        from_be32(&src[tsp..tsp + 4])
                    } - 8) as usize; // go to IFD0
                    let entries = if le {
                        from_le16(&src[tsp..tsp + 2])
                    } else {
                        from_be16(&src[tsp..tsp + 2])
                    }; // number of entries
                    tsp += 2;
                    for _i in 0..entries {
                        let tag = if le {
                        ...
                        tsp += 2;
                        if format > 12 {
                            return Err(Error("Invalid JPEG 8".to_string()));
                        }
                        let components = if le {
                            from_le32(&src[tsp..tsp + 4])
                        } else {
                            from_be32(&src[tsp..tsp + 4])
                        };
                        ...
                        match tag {
                            0x0106 => {
                                ...
                                let pe = if le {
                                    from_le16(&src[dsp..dsp + 2])
                                } else {
                                    from_be16(&src[dsp..dsp + 2])
                                };
                                if (pe != 2) || (itype != TYPE_YUV444) {
                                    return Err(Error("Invalid JPEG 9".to_string()));
                                }
                                itype = TYPE_RGB444;
                            }
                            0xA001 => { // colorspace
            ...
            _ => {
                ...
                match itype {
                    TYPE_Y => {
                        convert_blocks(&mut coeffs, mbtotal, TYPE_Y, &qtable, &qt);
                    }
                    TYPE_YUV420 => {
                        convert_blocks(&mut coeffs, mbtotal * 6, TYPE_YUV420, &qtable, &qt);
                    }
                    TYPE_YUV422 => {
                        convert_blocks(&mut coeffs, mbtotal * 4, TYPE_YUV422, &qtable, &qt);
                    }
                    TYPE_YUV440 => {
                        convert_blocks(&mut coeffs, mbtotal * 4, TYPE_YUV440, &qtable, &qt);
                    }
                    TYPE_YUV444 => {
                        convert_blocks(&mut coeffs, mbtotal * 3, TYPE_YUV444, &qtable, &qt);
                    }
                    TYPE_RGB444 => {
                        convert_blocks(&mut coeffs, mbtotal * 3, TYPE_RGB444, &qtable, &qt);
                    }
                    _ => {}
                }
                #[allow(unused_assignments)]
                ...
                return Ok(image);
                  ...
                  return Err(Error("Invalid JPEG 10".to_string());
              },*/
        }
        sp += length + 2;
    }
    Err(Error("Invalid JPEG 11".to_string()))
}
